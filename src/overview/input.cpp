...
bool HTManager::end_window_drag() {
    ...
    if (cursor_workspace == nullptr && workspace_id != WORKSPACE_INVALID) {
        ...
        Debug::log(
            LOG,
            "[Hyprtile] Dragging to invalid position, snapping to last ws {}",
            cursor_workspace->m_id
        );
    }

    if (cursor_workspace == nullptr) {
        Debug::log(LOG, "[Hyprtile] tried to drop on null workspace??");
        g_pKeybindManager->changeMouseBindMode(MBIND_INVALID);
        return false;
    }

    Debug::log(LOG, "[Hyprtile] trying to drop window on ws {}", cursor_workspace->m_id);
...
bool HTManager::swipe_update(IPointer::SSwipeUpdateEvent e) {
    ...
    const int ENABLED = HyprtileConfig::value<Hyprlang::INT>("gestures:enabled");
    if (!ENABLED)
        return false;

    const unsigned int MOVE_FINGERS = HyprtileConfig::value<Hyprlang::INT>("gestures:move_fingers");
    const float OPEN_DISTANCE = HyprtileConfig::value<Hyprlang::FLOAT>("gestures:open_distance");
    const unsigned int OPEN_FINGERS = HyprtileConfig::value<Hyprlang::INT>("gestures:open_fingers");
    const int OPEN_POSITIVE = HyprtileConfig::value<Hyprlang::INT>("gestures:open_positive");
