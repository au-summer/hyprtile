...
void HTLayoutColumn::rebuild_columns() {
    ...
    std::sort(columns.begin(), columns.end(),
              [](const auto& a, const auto& b) { return a.column_id < b.column_id; });
}
...
CBox HTLayoutColumn::calculate_ws_box(int col_idx, int row_idx, HTViewStage stage) {
    ...
    return CBox{Vector2D{col_idx, row_idx} * (ws_sz + Vector2D{GAP_SIZE, GAP_SIZE}) + Vector2D{GAP_SIZE, GAP_SIZE} + use_offset + start_offset, ws_sz};
}
