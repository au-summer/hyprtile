...
void HTView::change_layout(const std::string& layout_name) {
    ...
    // Only column layout is supported for hyprtile
    if (layout_name == "column") {
        layout = makeShared<HTLayoutColumn>(monitor_id);
    } else {
        fail_exit(
            "Bad overview layout name {}, only 'column' is supported for hyprtile",
            layout_name
        );
    }
}
...
void HTView::do_exit_behavior(bool exit_on_mouse) {
    ...
    const int EXIT_ON_HOVERED = HyprtileConfig::value<Hyprlang::INT>("exit_on_hovered");
...
void HTView::move_id(WORKSPACEID ws_id, bool move_window) {
    ...
    if (move_window) {
        ...
        warp = *CConfigValue<Hyprlang::INT>("plugin:hyprtile:overview:warp_on_move_window");
    } else {
...
PHLMONITOR HTView::get_monitor() {
    ...
    if (monitor == nullptr)
        Debug::log(WARN, "[Hyprtile] Returning null monitor from get_monitor!");
    return monitor;
}
